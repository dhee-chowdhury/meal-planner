...
const ServingUnitCards = () => {
  const { updateSelectedServingUnitId, updateServingUnitDialogOpen } = useServingUnitStore();
  ...
  if (servingUnitsQuery.data?.length === 0) {
    return <NoItemsFound onClick={() => updateServingUnitDialogOpen(true)} name="Serving Unit" />;
  }
